...
# rounding helpers
def r2(x): return float(round(float(x or 0), 2))
def r3(x): return float(round(float(x or 0), 3))
def r6(x): return float(round(float(x or 0), 6))
...
def save_checkpoint(agent, tag: str):
        ...
        try:
            ...
            if BACKTEST_WHEN_CLOSED:
                ...
                with STATE_LOCK:
                    ...
                    STATE.update({
                        'block_risk': block_risk, 'block_cash': eq, 'equity': eq,
                        'today_trades': 0, 'today_pnl': 0.0, 'cash': eq,
                ...
                for _ in range(2000):
                    ...
                    insert_trade({
                        ...
                        "risk_frac": risk, "position_before": 0.0, "position_after": 0.0,
                        "cost_basis_before": 0.0, "cost_basis_after": 0.0,
                        "realized_pnl": r2(pnl if a != env.cash_idx else 0.0),
                        "realized_pnl_pct": r3((pnl/abs(notional)) if (a != env.cash_idx and notional!=0) else 0.0),
                        "equity_after": r2(eq), "reason": "exit"
                    })

                    with STATE_LOCK:
                        STATE['today_trades'] += 2 if a != env.cash_idx else 1
                        STATE['today_pnl'] = r2(STATE.get('today_pnl',0.0) + (pnl if a != env.cash_idx else 0.0))
