...
# ===================== ENV / PPO (unchanged structure; shortened for brevity) =====================
@dataclass
...
# (identical to previous message you have; omitted here for brevity in this box)
# NOTE: keep your existing trainer_thread and live_thread implementations
# from the last message; no behavioral change required here.
# -------------------- (copy them from your current file) --------------------

# For brevity in this response, keep your existing trainer_thread/live_thread code block as-is.
# ===================== API =====================
app=FastAPI()
...
def equity(window:str="30d",session:Optional[str]=None,bench:Optional[str]=None):
    ...
    try:
        ...
        if raw:
            # align to series & scale to equity start
...
def snapshot(_:None=Depends(require_key)):
    ...
    try:
        env,_,_=fetch_env_for_snapshot()
        return {"ok":True}
    except Exception as e:
        raise HTTPException(500, detail=str(e))

def fetch_env_for_snapshot():
    env,_,_=build_env()
...
def admin_pause(req:PauseReq,_:None=Depends(require_key)):
    if req.password!=API_KEY and req.password!=os.getenv("ADMIN_RESET_PASSWORD","please-change-me"): raise HTTPException(403, detail="Bad admin password")
    PAUSED.set(); with STATE_LOCK: STATE['paused']=True; STATE['status']='paused'
    return {"ok":True,"paused":True}

@app.post("/admin/resume")
def admin_resume(req:PauseReq,_:None=Depends(require_key)):
    if req.password!=API_KEY and req.password!=os.getenv("ADMIN_RESET_PASSWORD","please-change-me"): raise HTTPException(403, detail="Bad admin password")
    PAUSED.clear(); with STATE_LOCK: STATE['paused']=False
...
# ====== Boot threads (use your existing trainer_thread/live_thread implementations) ======
